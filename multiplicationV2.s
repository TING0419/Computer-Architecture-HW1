...
convert_fp32_to_bf16:
    ...
    # 提取 input1 的指數和尾數
    and t0, s0, s2    # t0 = input1 exponent
    and t1, s0, s3    # t1 = input1 mantissa
    bnez t0, exp1_non_zero  # 如果指數不為零，跳到下一段
    beqz t1, return_result  # 如果尾數為零，直接返回
...
exp1_non_zero:
    ...
    # 提取 input2 的指數和尾數
    and t0, s1, s2    # t0 = input2 exponent
    and t1, s1, s3    # t1 = input2 mantissa
...
calculate:
    ...
    lw s4, 0(s4)
    and t3, s0, s3    # t3 = input1 mantissa
    and t4, s1, s3    # t4 = input2 mantissa
    srli t3, t3, 16
    srli t4, t4, 16
    or t3, t3, s4
    or t4, t4, s4
    mv s9, t3         # s9 保存 input1 的尾數（未移位）
    mv t4, t4         # t4 為 input2 的尾數

    # 初始化乘法相關變量
    mv t5, x0         # 結果的累加器
    mv t3, s9         # t3 = input1 mantissa
